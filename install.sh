...
#
# contextTokens — сколько офис держит «на столе» в одном разговоре. Стояло 48000
# с самого первого установщика, без расчёта. На живом офисе замер показал 25k из
# 48k занятыми в обычной переписке: больше половины окна съедали инструкции офиса,
# на работу оставалось меньше половины небольшого стола — и человек видел, что
# офис забывает начало разговора.
#
# Модели держат кратно больше: gemini-2.5-flash 1 048 576, claude-sonnet-5 1 000 000
# (проверено 30.07 у openrouter.ai/api/v1/models). Ставим 200000 — вчетверо больше
# прежнего и вшестеро меньше потолка модели.
#
# Почему не миллион: при каждой реплике на сервер модели уходит весь стол целиком,
# и клиент платит за него заново. Цена входа gemini-2.5-flash — $0.30 за 1 млн
# токенов (там же, 30.07), то есть полный стол на 200k стоит около 6 центов
# за сообщение в худшем случае. Миллион стоил бы 30 центов за реплику — при
# ежедневной работе это заметные деньги за память, которой человек не просил.
